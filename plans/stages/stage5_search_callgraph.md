...
> **实现修订（阶段 5 实现时）**：后端 callers/callees API 返回纯 `SymbolOut[]`，不携带 `callee_id`/精确性元信息。在零引擎改动约束下，无法区分单条边的精确性。降级为：所有边统一样式 + 调用图/剥洋葱全局诚实提示文案「按名匹配，可能不准」。§9.1 验收清单「调用图边区分实线/虚线」调整为「调用图显示诚实提示」。
...
- [x] 用户批准
- [x] 14 个 TDD 循环逐一执行（红绿重构）
- [x] cc 验收 + 修复全部反馈（2 严重 + 4 高价值建议）
- [ ] 用户验收
- [ ] push
